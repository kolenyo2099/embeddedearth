...
class CopernicusFMViT(nn.Module):
    ...
    def forward_features(self, x, meta_info, wave_list, bandwidth, language_embed, input_mode, kernel_size=None):
        ...
        if input_mode == 'spectral':
            ...
            self.waves = wavelist
            x, (h, w), _ = self.patch_embed_spectral(x, self.waves, bandwidths, kernel_size)
        elif input_mode == 'variable':
            x, (h, w), _ = self.patch_embed_variable(x, language_embed, kernel_size)

        # resize pos embed
        num_patches = x.size(1)
        # num_patches_sqrt = int(math.sqrt(num_patches))
        num_patches_sqrt_origin = int(math.sqrt(self.num_patches))
        pos_embed = resize_abs_pos_embed(self.pos_embed, (h, w), (num_patches_sqrt_origin,num_patches_sqrt_origin), num_prefix_tokens=1)
